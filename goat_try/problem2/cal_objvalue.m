...
global Phase_sin
global Mag_pow10
range_AZ_min_index=(-20+180)/5+1;%范围水平角最小边界
range_AZ_max_index= (20+180)/5+1;%范围水平角最大边界
range_EL_min_index=(-10+90)/5+1;%范围俯仰角最小边界
range_EL_max_index= (10+90)/5+1;%范围俯仰角最大边界
for j = 1 : 100
mix_field_average=0;
...
mix_field_value=zeros(360/5+1,180/5+1);
 ...
 %   display(pop(j,1));
   ...
   % display(ans1);
   % display(ans2);
    objvalue(1,j) = 1000/(1+exp(-(mix_field_average-30)));%exp(mix_field_average-20);%对于特定的第一问的两个位置进行计算；
